...
/** What a generated icon component looks like. Every file in
 *  `src/assets/system_icons` matches this, including the two statics. */
export type Glyph = ComponentType<{ size?: number; strokeWidth?: number; className?: string }> & {
  grid?: number;
  weight?: number;
};
 ...
 *
 * Stroke needs the icon's DRAWN size and its DRAWN weight, both of which the
 * generated component carries. Figma's `Size` variant is 16, 20 or 32 and its
 * `Weight` variant is 1px or 2px; rendering at the drawn size passes the weight
 * straight through, and any other size rescales it (see `iconStroke`).
 *
...
export function Icon({
  ...
  label?: string;
  /** Figma's `Weight` variant, in pixels, when a call site places a variant
   *  other than the one the component was generated from. Rare — the drawn
   *  weight travels with the glyph. */
  weight?: number;
}) {
  ...
  return (
    <span
      ...
      aria-label={label}
    >
      <Glyph size={size} strokeWidth={iconStroke(size, grid, weight ?? Glyph.weight ?? 1)} />
    </span>
  );
}
